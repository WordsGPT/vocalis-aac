...
import { Mic, MicOff, Volume2, User, Radio, Trash2, Sparkles, AudioWaveform, RotateCcw, ArrowUp } from 'lucide-react';

const QUICK_TEST_PROMPTS = [
  "Hey! Do you want to eat some cheese?",
  "Awesome! Do you prefer cheddar or gouda?",
    ...
    <div className="w-full bg-slate-900/70 rounded-2xl border border-slate-800 shadow-2xl backdrop-blur-md overflow-hidden flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-5 py-3.5 bg-slate-900/90 border-b border-slate-800">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-[11px] text-slate-400 font-medium mr-1 hidden sm:inline">Try Demo:</span>
      ...
      {/* Scrollable Conversation Stream (Greyed-out past messages scroll up) */}
      <div
      ...
      >
        {history.length > 2 && (
          <div className="flex items-center justify-center gap-1.5 py-1 text-[11px] text-slate-600 font-medium">
            ...
            <span>Earlier in conversation (scroll up to view)</span>
          </div>
        )}

        {/* 1. PAST MESSAGES (Visually Greyed Out / Muted) */}
          ...
          return (
            <div
              ...
              className={`group flex items-start gap-2.5 p-3 rounded-xl border text-sm transition-all duration-200 ${
                isOlder
                  ? 'opacity-45 hover:opacity-100 bg-slate-900/30 border-slate-800/40 text-slate-400'
                  : 'opacity-85 bg-slate-900/60 border-slate-800 text-slate-300'
        ...
        {/* 2. CURRENT ACTIVE TURN (In Sharp Contrast & Full Brightness) */}
          ...
          <div className="p-4 rounded-xl bg-slate-900/90 border border-blue-500/40 shadow-lg shadow-blue-500/5 ring-1 ring-blue-500/20">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-blue-300">
                  {displayLiveText ? 'Partner is saying right now:' : 'Listening for partner...'}
                </span>
              </div>
            ...
            {displayLiveText ? (
              ...
              <div className="flex items-center gap-2 text-slate-500 italic text-sm py-1.5 text-left">
                ...
                <span>
                  {isListening
                    ? 'Speak into microphone or select a demo prompt above...'
                    : 'Microphone paused. Click "Listen to Partner" above to begin.'}
                </span>
              </div>
            )}
            ...
            {isTranscribing && (
              <div className="mt-2 text-xs text-blue-400 flex items-center gap-1.5 animate-pulse">
                <span>Whisper GPU Transcribing audio...</span>
              </div>
            )}
